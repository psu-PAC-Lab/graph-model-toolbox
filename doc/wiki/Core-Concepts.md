...
- A set of **directed edges** $E$ — representing energy flows between them
...
An edge represents a flow between two vertices, described by a **flow equation** that may depend on head/tail vertex states, inputs, and parameters.
...
Flow is **positive** when entering the head vertex. Vertex index `0` is the conventional ground/reservoir node.
...
| Type | Meaning |
|------|---------|
| `Internal` | Flow computed from vertex states (default) |
| `External` | Flow driven by a user-supplied input `u` |
