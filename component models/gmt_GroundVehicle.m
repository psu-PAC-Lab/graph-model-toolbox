...
classdef gmt_GroundVehicle < gmt_Graph
    ...
    methods
        ...
        function obj = gmt_GroundVehicle(ObjectName,varargin)
            ...
            % Define Vertex 
            Vertex(1) = gmt_Vertex("Vehicle Velocity","(A*x2*x2*(D/2)*(D/2) + B*x2*(D/2) + C)*x","units","m/s");
            Vertex(2) = gmt_Vertex("Source Angular Velocity","x","External",true,"units","rad/s");

            % Define Edge 
            Edge(1) = gmt_Edge("Vehicle Road Load Power","xt*(D/2)*(A*xt*xt*(D/2)*(D/2) + B*xt*(D/2) + C)");
            ...
            Parameter(4) = gmt_Parameter("Tire Diameter","D",0.64,"units","m");
