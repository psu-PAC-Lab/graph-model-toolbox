...
classdef gmt_Inverter < gmt_Graph
    ...
    methods
        ...
        function obj = gmt_Inverter(ObjectName)
            ...
            % Define Vertices 
            Vertices(1) = gmt_GraphVertex("Input Voltage","x","External");
            Vertices(2) = gmt_GraphVertex("Output Voltage","x");
            Vertices(3) = gmt_GraphVertex("Input Current","x","External");
            Vertices(4) = gmt_GraphVertex("Output Current","x");
            Vertices(5) = gmt_GraphVertex("Inverter Temperature","Cp*x_dot");

            % Define Edges 
            Edges(1) = gmt_GraphEdge("Voltage Input to Voltage Output","sqrt(3/2)*xt*u1");
            Edges(2) = gmt_GraphEdge("Current Input to Voltage Output","Rm*xh*u1");
            Edges(3) = gmt_GraphEdge("Current Input to Current Output","sqrt(3/2)*xt*u1");
            Edges(4) = gmt_GraphEdge("Electrical to Thermal","xt*xt");
